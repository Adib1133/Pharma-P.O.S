...
(function () {
  ...
  // Application State
  const state = {
    activeUser: { id: 2, name: 'Alex Carter', role: 'cashier' },
  ...
  function setupBarcodeWedgeListener() {
    window.addEventListener('keydown', (e) => {
      ...
      if (document.activeElement && document.activeElement.tagName === 'INPUT' && document.activeElement.id !== 'pos-search-input') {
        return;
      }
    ...
    document.getElementById('btn-pin-submit').addEventListener('click', async () => {
      ...
      try {
        ...
        if (res.ok && data.success) {
          ...
          state.pinBuffer = '';
          showToast(`Authenticated as ${data.user.name}`, 'success');
          await checkRegisterSession();
        } else {
