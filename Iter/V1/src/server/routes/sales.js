...
router.post('/checkout', async (req, res) => {
  try {
    ...
    const invoice_no = `INV-${dateStr}-${randomSeq}`;
    ...
    const saleResult = await run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      invoice_no, cashier_id || 1, customer_id || 1, subtotal, discount_amount || 0,
      tax_amount, grand_total, payment_method, JSON.stringify(payment_details || {}),
      cash_given || grand_total, change_due || 0, 'completed'
    ]);
    ...
    for (const item of items) {
      await run(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        saleId, item.medicine_id, item.batch_id, item.qty, item.unit_price,
        item.gst_percent || 12, item.gst_amount || 0, item.total_amount
      ]);

      // Deduct inventory
      await run('UPDATE stock_batches SET qty_remaining = qty_remaining - ? WHERE id = ?', [item.qty, item.batch_id]);

      // Audit Log
      await run(`
        INSERT INTO stock_audit_log (batch_id, medicine_id, change_qty, reason, user_id)
        VALUES (?, ?, ?, ?, ?)
      `, [item.batch_id, item.medicine_id, -item.qty, `Sale ${invoice_no}`, cashier_id || 1]);
    }
    ...
    if (customer_id && customer_id > 1) {
      ...
      if (pointsEarned > 0) {
        await run('UPDATE customers SET loyalty_points = loyalty_points + ? WHERE id = ?', [pointsEarned, customer_id]);
      }
    }
...
router.get('/', async (req, res) => {
  try {
    const { start_date, end_date, cashier_id, payment_method, q } = req.query;
    let sql = `
...
router.post('/held', async (req, res) => {
  try {
    ...
    const result = await run(`
      ...
      VALUES (?, ?, ?)
    `, [cashier_id || 1, customer_name || 'Walk-in Customer', JSON.stringify(cart_data)]);
