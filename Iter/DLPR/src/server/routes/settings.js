...
// Resolve writable upload path relative to exe (not snapshot) when packaged
...
router.post('/logo', upload.single('logo'), async (req, res) => {
  ...
  try {
    ...
    // Logos are served from the public/uploads folder next to the exe
    const targetDir = isPackaged
      ? path.join(appRoot, 'src', 'public', 'uploads')
