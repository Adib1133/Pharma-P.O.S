...
router.post('/open', async (req, res) => {
  try {
    ...
    if (!cashier_id) {
      return res.status(400).json({ error: 'Cashier ID is required to open a register session' });
    }

    const cid = cashier_id;
    const openFloat = parseFloat(opening_balance) || 0;

    // Close any unclosed lingering session for this cashier
    await run(
      `UPDATE cash_register_sessions SET status = 'closed', closed_at = CURRENT_TIMESTAMP WHERE cashier_id = ? AND status = 'open'`,
      [cid]
    );
...
// FIX #11: Added session ownership check — only the cashier who opened the
// session (or an admin passing the same cashier_id) can close it.
// Previously, any caller with a session_id could close any session.
router.post('/close', async (req, res) => {
  try {
    const { session_id, cashier_id, closing_balance_actual, notes } = req.body;
    ...
    // Ownership check: verify the requesting cashier owns this session
    if (cashier_id && String(session.cashier_id) !== String(cashier_id)) {
      return res.status(403).json({ error: 'You can only close your own register session' });
    }
    ...
    const varianceNote = notes ||
      (Math.abs(variance) < 0.01 ? 'Exact count matches' : `Cash variance: Tk.${variance.toFixed(2)}`);
    ...
    await run(`
      UPDATE cash_register_sessions SET
        ...
        notes = ?
      WHERE id = ?
    `, [expectedCash, actualCash, varianceNote, session_id]);
